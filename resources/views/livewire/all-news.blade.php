        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            @foreach ($news as $key => $new)
                <tr>
                    ...
                    <td title="{{ $new->desc }}" class="truncate-text-3">{{ $new->desc }}</td>
                    <td>{{ $new->tags }}</td>
                    <td>{{ $new->admin->name }}</td>
                    <td>{{ $new->view_count }}</td>
                    <td>{{ $new->comment_count }}</td>
                    ...
                    <td>
                        <button type="button" class="btn btn-primary" wire:click="editRecord({{ $new->id }})">
                            <i class="fa-solid fa-pen-to-square"></i>
                        </button>
                        <button onclick="return confirm('Bạn chắc chắn muốn xóa các bình luận này?') || event.stopImmediatePropagation()" type="button" class="btn btn-danger" wire:click="deleteRecord({{ $new->id }})">
