<div class="card-transparent text-dark" x-data="{ edit: false }">
    <div class="card-body p-4">
        <div class="d-flex">
            <img class="rounded-circle shadow-1-strong me-3" src="{{ $comment->user->avatar_url }}" alt="avatar"
                width="60" height="60" />
            <div class="flex-fill">
                ...
                @if ($showCommentForm)
                    <div class="mt-3">
                        <livewire:user.comment-form :postId="$comment->post_id" :replyId="$comment->id" />
                    </div>
                @endif
                ...
                @if ($showReplies)
                    @forelse($comment->replies as $reply)
                        <livewire:user.load-comment :comment="$reply" :key="$reply->id" />
                    @empty
                    @endforelse
                @endif
            </div>
