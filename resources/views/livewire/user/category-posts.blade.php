<div>
    <div class="tag">
        ...
        <button wire:click="setSortBy('view_count')" @class(['badge', 'focus' => $sortBy === 'view_count'])>
            Lượt xem
            <i class="ms-1 fa-solid fa-sm {{ $sortBy !== 'view_count' ? 'fa-sort' : ($sortDirection === 'asc' ? 'fa-sort-amount-up-alt' : 'fa-sort-amount-down-alt')}}"></i>
        </button>

        <button wire:click="setSortBy('like_count')" @class(['badge', 'focus' => $sortBy === 'like_count'])>
            Lượt thích
            <i class="ms-1 fa-solid fa-sm {{ $sortBy !== 'like_count' ? 'fa-sort' : ($sortDirection === 'asc' ? 'fa-sort-amount-up-alt' : 'fa-sort-amount-down-alt')}}"></i>
        </button>

        <button wire:click="setSortBy('comments_count')" @class(['badge', 'focus' => $sortBy === 'comments_count'])>
            Lượt bình luận
            <i class="ms-1 fa-solid fa-sm {{ $sortBy !== 'comments_count' ? 'fa-sort' : ($sortDirection === 'asc' ? 'fa-sort-amount-up-alt' : 'fa-sort-amount-down-alt')}}"></i>
        </button>

        <button wire:click="setSortBy('min_price')" @class(['badge', 'focus' => $sortBy === 'min_price'])>
            Giá
            <i class="ms-1 fa-solid fa-sm {{ $sortBy !== 'min_price' ? 'fa-sort' : ($sortDirection === 'asc' ? 'fa-sort-amount-up-alt' : 'fa-sort-amount-down-alt')}}"></i>
        </button>

        <button wire:click="setSortBy('star')" @class(['badge', 'focus' => $sortBy === 'star'])>
            Sao
            <i class="ms-1 fa-solid fa-sm {{ $sortBy !== 'star' ? 'fa-sort' : ($sortDirection === 'asc' ? 'fa-sort-amount-up-alt' : 'fa-sort-amount-down-alt')}}"></i>
        </button>
    </div>
                    ...
                    <div class="col-md-8">
                        <div class="card-body pt-0">
                            ...
                            <p class="mb-0" data-bs-toggle="tooltip"
                                title="{{ $post->best_star_place->starTooltip }}" data-bs-placement="left">
